...
class OIDC_Admin {
	...
	public function render_settings_page(): void {
		...
		jQuery(document).ready(function($) {
			$('#oidc-discover-btn').on('click', function(e) {
				...
				// Fetch the OIDC discovery document via AJAX
				$.ajax({
					url: ajaxurl,
					type: 'POST',
					data: {
	...
	public function render_role_field( array $args ): void {
		$options = get_option( 'secure_oidc_login_settings', array() );
		$field   = $args['field'];
		$value   = isset( $options[ $field ] ) ? $options[ $field ] : 'subscriber';
	...
	public function ajax_discover(): void {
		...
		$discovery_url = isset( $_POST['discovery_url'] ) ? esc_url_raw( $_POST['discovery_url'] ) : '';
